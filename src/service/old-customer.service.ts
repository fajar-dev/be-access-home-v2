...
import type { RawSnapshotInput } from "../interface/snapshot.interface";
import {
  findOldCustomerAccounts,
  findOldCustomerInvoices,
...
// so a re-run never touches the new-customer rows for the same period.
...
export async function fetchOldCustomerSnapshotInputs(
  period: string,
  ...
  const invoiceRows = await findOldCustomerInvoices([
    period,
    startStr, endStr,
    startStr, endStr,
    startStr, endStr,
  ]);
  const accountRows = await findOldCustomerAccounts();
